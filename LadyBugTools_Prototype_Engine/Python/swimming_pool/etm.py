...
def equibtemp1(
    ...
    if solrad < 0.0:
        raise ValueError("downwelling solar radiation must be greater than or equal to 0")
...
def equibtemp(
    ...
    tau = (rho_w*cw*depth)/(4*sigma*(t_wb**3)+wf*(delcalc(t_wb-273.15)+gamma)) #seconds
    
    t_equib = t_wb + (net_rad_wb)/(4*sigma*(t_wb**3)+wf*(delcalc(t_wb-273.15)+gamma)) #K
    ...
    lambda_e = (delcalc(t_air-273.15)*(net_rad_t0 - N) + rho_a*c_a*(vpdcalc(saturated_vapor_pressure(t_air), rh)/ra)) / (delcalc(t_air-273.15) + gamma) #J
