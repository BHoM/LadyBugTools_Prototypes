...
from ladybugtools_toolkit.helpers import sunrise_sunset
from ladybugtools_toolkit.plot import heatmap
...
def cloud_cover_categories(epw: EPW, ax: plt.Axes = None) -> plt.Axes:
    ...
    mtx = (
        s.groupby([s.index.month, s.index.day], axis=0)
        .value_counts()
        .unstack()
        .fillna(0)
    )
    ...
    mtx = mtx.groupby(mapper, axis=1).sum()[
